    ...
    return new Promise((resolve, reject) => {
        ...
        form.submit({
            ...
            if (err) {
                ...
                // console.log('resieve: ' + filename + ' with Statuscode ' + res.statusCode)
                resolve({
...
/**
 * send one grpup of requests parallel after a delay time
 * @param  {Intiger} groupSize number of requests
 * @param  {Intiger} delay time in ms
 * @param {Function} [request] Callback function to send requests
 * @return {Promise.Array} [responds] [{statusCode, fileName, requestTime, deckId, noOfSlides}, {} ...]
 */
let groupedTimedRequest = (groupSize, breakTime, data, request) => {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            // console.log('\n##' + groupSize + ' requests to send ## ' , now().format('HH:mm:ss'))
            let array = [];
            ...
            Promise.all(dataId.map((id) => {
                return request(data[id][0], data[id][1]);
            })).then((responds) => {
                resolve(responds);
            }).catch((reason) => {
                reject(reason);
...
module.exports = {
    ...
    summary: summary,
    groupedTimedRequest: groupedTimedRequest,
};